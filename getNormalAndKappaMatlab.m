function vec = getNormalAndKappa(node,listNeigh,X,Y,Z)
 ...
 for i=1:listSize-1
  ...
  ad = [x1Unit y1Unit z1Unit];
  bd = [x2Unit y2Unit z2Unit];
  cd = cross(ad,bd);

  sumXCrossUnit = sumXCrossUnit + cd(1);
  sumYCrossUnit = sumYCrossUnit + cd(2);
  sumZCrossUnit = sumZCrossUnit + cd(3);
  ...
  %                PlaneNormal
  %
  retaNorm = norm([xReta yReta zReta],2);
  xRetaUnit = xReta/retaNorm;
  yRetaUnit = yReta/retaNorm;
  zRetaUnit = zReta/retaNorm;
  dotReta = dot([xPlaneRes yPlaneRes zPlaneRes],[xRetaUnit yRetaUnit zRetaUnit]);
  
  xPlaneTang = xRetaUnit*dotReta;
  yPlaneTang = yRetaUnit*dotReta;
  zPlaneTang = zRetaUnit*dotReta;

  xPlaneNormal = xPlaneRes - xPlaneTang;
  yPlaneNormal = yPlaneRes - yPlaneTang;
  zPlaneNormal = zPlaneRes - zPlaneTang;

  len = norm([xPlaneNormal yPlaneNormal zPlaneNormal],2);
  ...
  sumArea = sumArea + (1.0/3.0)*triangle_area([[P0x P0y P0z];[P1x P1y P1z];[P2x P2y P2z]],'h');
 end
 len = norm([sumXCrossUnit sumYCrossUnit sumZCrossUnit],2);
 xNormalUnit = sumXCrossUnit/len;
 yNormalUnit = sumYCrossUnit/len;
 zNormalUnit = sumZCrossUnit/len;

 % intensidade da forca resultante
 force = -norm([fx fy fz],2);
 
 % teste para saber o sentido correto de aplicacao da
 % pressao no noh.
 if( dot([fx fy fz],[xNormalUnit yNormalUnit zNormalUnit]) < 0.0 )
  force = -force;
 end
